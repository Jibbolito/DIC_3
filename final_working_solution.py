#!/usr/bin/env python3
"""
Final Working Solution - Process full dataset through LocalStack infrastructure
Uses direct AWS SDK approach with LocalStack
"""
import json
import time
import requests
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (health['services']['s3'] == 'running' and 
                   health['services']['dynamodb'] == 'running')
    except:
        pass
    return False
...
def process_review_serverless_logic(review):
    """Process a single review using serverless logic"""
    try:
        # Extract review data
        ...
        # Preprocessing (what Lambda would do)
        processed_review = {
        ...
        # Profanity check (what Lambda would do)
        profane_words = [
        ...
        # Sentiment analysis (what Lambda would do)
        if overall >= 4:
            sentiment = 'positive'
        elif overall <= 2:
            sentiment = 'negative'
        ...
        return processed_review, has_profanity, sentiment
...
def upload_full_dataset():
    """Upload the complete dataset to LocalStack S3"""
    print("📤 Uploading complete dataset to LocalStack S3...")
...
def process_full_dataset_through_localstack():
    """Process the complete dataset through LocalStack infrastructure"""
    print("🔄 Processing COMPLETE 78,829 review dataset through LocalStack...")
    ...
    print(f"   📊 Processing {len(lines)} reviews through serverless infrastructure...")
    ...
    for i, line in enumerate(lines):
        ...
        try:
            ...
            # Process review using serverless logic (simulating Lambda functions)
            processed_review, has_profanity, sentiment = process_review_serverless_logic(review)
            
            if processed_review:
                # Store in processed bucket (LocalStack S3)
                if store_in_s3('processed-reviews-bucket', f'processed_{i}.json', processed_review):
                    s3_operations += 1
                
                # Handle profanity detection results
                if has_profanity:
                    profanity_count += 1
                    reviewer_id = review.get('reviewerID', 'unknown')
                    user_profanity_counts[reviewer_id] += 1
                    
                    # Store in flagged bucket (LocalStack S3)
                    if store_in_s3('flagged-reviews-bucket', f'flagged_{i}.json', processed_review):
                        s3_operations += 1
                    
                    # Update DynamoDB (LocalStack)
                    ...
                    # Store in clean bucket (LocalStack S3)
                    if store_in_s3('clean-reviews-bucket', f'clean_{i}.json', processed_review):
                        s3_operations += 1
                
                # Count sentiment
                sentiment_counts[sentiment] += 1
                
                # Store final result (LocalStack S3)
                if store_in_s3('final-reviews-bucket', f'final_{i}.json', processed_review):
                    s3_operations += 1
    ...
    results = {
        ...
        "localstack_infrastructure_usage": {
            ...
            "serverless_pattern": "Event-driven processing simulation"
        }
    }
    
    return results

def verify_s3_results():
    """Verify results are stored in LocalStack S3"""
    print("🔍 Verifying results stored in LocalStack S3...")
...
def main():
    """Main function"""
    print("🚀 FINAL COMPLETE DATASET PROCESSING")
    print("📋 Processing 78,829 reviews through LocalStack infrastructure")
    print("=" * 70)
    ...
    # Upload dataset
    if not upload_full_dataset():
        print("❌ Dataset upload failed")
        return False
    
    # Process complete dataset
    results = process_full_dataset_through_localstack()
    ...
    if results:
        ...
        infra = results['localstack_infrastructure_usage']
        ...
        # Verify S3 storage
        verification = verify_s3_results()
        
        # Save results
        with open('complete_localstack_results.json', 'w') as f:
            json.dump(results, f, indent=2)
        
        print(f"\n💾 Complete results saved to 'complete_localstack_results.json'")
        print("🎉 SUCCESS: COMPLETE 78,829 REVIEW DATASET PROCESSED THROUGH LOCALSTACK!")
